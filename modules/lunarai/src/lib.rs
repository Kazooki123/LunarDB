...
pub fn lunar_ai_create() -> LunarAIHandle {
    let lunar_ai = Box::new(LunarAI::new());
    LunarAIHandle(Box::into_raw(lunar_ai))
}

pub fn lunar_ai_destroy(handle: LunarAIHandle) {
    if !handle.0.is_null() {
        unsafe {
...
pub fn lunar_ai_version() -> *const c_char {
    let version = CString::new(LunarAI::version()).unwrap();
    version.into_raw()
}

pub fn lunar_ai_generate_embedding(
...
pub fn lunar_ai_compute_similarity(
    handle: LunarAIHandle,
...
pub fn lunar_ai_free_string(ptr: *mut c_char) {
    unsafe {
        if !ptr.is_null() {
